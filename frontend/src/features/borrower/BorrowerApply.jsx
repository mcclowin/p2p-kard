...
import { FileUp, Check, X, Hourglass, Handshake, Mail } from "lucide-react";
                ...
                {!noDocuments && (
                  <>
                    <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
                      ...
                      <FileUp className="w-8 h-8 mx-auto text-emerald-500" />
          ...
          <FadeIn>
            <div className="text-center space-y-6 py-8">
              <div className={`inline-flex w-20 h-20 rounded-full items-center justify-center text-4xl ${
                ...
                {validationResult?.decision === "PASS" ? <Check className="w-10 h-10 text-emerald-600" /> : validationResult?.decision === "DECLINE" ? <X className="w-10 h-10 text-red-600" /> : <Hourglass className="w-10 h-10 text-blue-600" />}
              </div>
              ...
              {submittedBorrowRequestId && !endorseSent && (
                <div className="max-w-md mx-auto w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 space-y-3">
                  <h3 className="text-base font-bold font-heading text-center flex items-center justify-center gap-2"><Handshake className="w-5 h-5 text-emerald-600" /> Strengthen your request</h3>
              ...
              {endorseSent && (
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800 max-w-md mx-auto text-center">
                  <span className="inline-flex items-center gap-1.5"><Mail className="w-4 h-4" /> Invitation sent!</span> Your endorser will receive an email with a link to vouch for your request.
                </div>
              )}
